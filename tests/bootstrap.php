...
if ( ! file_exists( $_phpunit_polyfills_lib ) ) {
	echo "Could not find $_phpunit_polyfills_lib, have you run `docker-compose up` in order to install Composer packages?" . PHP_EOL; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	exit( 1 );
}
...
function _manually_load_plugin() {
	require dirname( dirname( __FILE__ ) ) . '/woostore-binary-bin-widget.php';
}
